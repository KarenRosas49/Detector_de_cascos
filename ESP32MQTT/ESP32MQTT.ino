...
IPAddress server(52,28,68,205);

WiFiClient espClient;
PubSubClient client(espClient);

//Variables
const int Acceso = 2;  // Para indicar el estatus de conexión
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "proyecto/escuchador") {  // En caso de recibirse mensaje en el tema codigoIoT/ejemplo/mqttin
    if(messageTemp == "TRUE"){
      ...
      if (contador == 40) {
        ...
        client.publish("proyecto/parte2/detector_cascos","FALSE");
        contador = 0;
      }
      
    }// fin del if (String(topic) == "proyecto/escuchador")
    else{
      Serial.println("No");
...
void reconnect() {
  ...
  while (!client.connected()) { // Pregunta si hay conexión
    ...
    if (client.connect("ESP32CAMClient")) { //Pregunta por el resultado del intento de conexión
      ...
      client.subscribe("proyecto/escuchador"); // Esta función realiza la suscripción al tema
